...
class MailTemplate(models.Model):
    ...
    def _debrand_translated_words(self):
        ...
        sent_by = _get_translated("Sent") or _("Sent")
        using_word = _get_translated("using") or _("using")
        using_word = _get_translated("using") or _("using")
        return odoo_word, powered_by, using_word,sent_by

    @api.model
    def _debrand_body(self, html):
        odoo_word, powered_by, using_word,sent_by = self._debrand_translated_words()
        html = re.sub(using_word + "(.*)[\r\n]*(.*)>" + odoo_word + r"</a>", "", html)
        if powered_by not in html and  sent_by not in html:
            return html
        root = htmltree.fromstring(html)
        powered_by_elements = root.xpath("//*[text()[contains(.,'%s')]]" % powered_by)
        sent_by_elements = root.xpath("//*[text()[contains(.,'%s')]]" % sent_by)
